# NOTE: this is a verbatim mirror of the file contributed to `imodels`
# (imodels/algebraic/binned_additive_gp.py, PR csinva/imodels#299). It imports
# two small helpers from imodels, so `pip install imodels` to run it directly;
# `addgp.py` in this folder is the self-contained research version.
#
# Unlike the research version it has no torch dependency: the marginal
# likelihood gradients are derived analytically in the sufficient statistics
